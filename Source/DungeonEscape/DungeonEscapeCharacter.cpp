...
void ADungeonEscapeCharacter::Interact()
{
	...
	FVector End = Start +(FirstPersonCameraComponent->GetForwardVector() * MaxInteractDist);

	FCollisionShape InteractionSphere = FCollisionShape::MakeSphere(InteractSphereRad);
		...
		else if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (LockActor) {
				...
				else
				{
					...
					// Add key item name into item list
					// Deactivate the lock
